...
def ripperUpper() -> int:
    ...
    nets_to_rip = set()             #Create a set to
    for i in range(NET_COUNT):
        if netList[i][6] == 0:  #If a net failed to route
            for j in range(2, RIPUP_MAX_LAYER):  #Scan all the layers above the start/end vias
                if layoutGrid[netList[i][0]][netList[i][1]][j][0] != -1:    #If there's something there
                    nets_to_rip.add(layoutGrid[netList[i][0]][netList[i][1]][j][0])
                if layoutGrid[netList[i][2]][netList[i][3]][j][0] != -1:    #If there's something there
                    nets_to_rip.add(layoutGrid[netList[i][2]][netList[i][3]][j][0])
